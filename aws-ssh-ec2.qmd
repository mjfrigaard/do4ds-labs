...
graph TD
    ...
    PubIP["Public IPv4: 203.0.113.42"]
    PubDNS["Public DNS: ec2-203-0-113-42..."]
    IGW["Internet Gateway"]
    VPC["VPC"]
    Subnet["Subnet 10.0.1.0/24"]
    Instance["EC2 Instance"]
    PrivIP["Private IPv4: 10.0.1.42"]
    PrivDNS["Private DNS: ip-10-0-1-42..."]

    Client -->|SSH port 22| PubIP
...
Press Enter when prompted for a passphrase (or add one for extra security). This creates two files: 

- `~/.ssh/<your-key-name>` (private key—keep this secret) 

- `~/.ssh/<your-key-name>.pub` (public key—share this with the server)

### Step 2. Add the public key to the server

Connect to the instance using the AWS `.pem` key:

```{bash}
#| eval: false
#| code-fold: false
ssh -i ~/.ssh/<your-aws-key-file-name>.pem ec2-user@YOUR_PUBLIC_IP_ADDRESS
```

Once logged in, create the `.ssh` directory and add the public key:
...
Change the permissions on the `authorized_keys`:
...
Now you can log in using your new key without needing the AWS `.pem` file:
...
Keep the AWS `.pem` file safe but don't use it for routine work. If the regular key is compromised, delete it from the server and generate a new one. The AWS key remains your recovery method.

## Using the Public DNS instead
...
#| eval: false 
#| code-fold: false 
ssh -i ~/.ssh/<your-key-file-name>.pem ec2-user@ec2-203-0-113-42.compute-1.amazonaws.com
```

The default public DNS is convenient for one-off connections, but note that it encodes the public IP and changes when the instance stops and restarts. For stable, persistent access, assign an Elastic IP address to the instance.
