...
graph TD
    subgraph "Python Decorators"
        A("<code>@reactive.calc</code>") --> B[Function<br>definition]
        C("<code>@reactive.event</code>") --> D["Event-driven<br>reactives"]
        E("<code>@render.text</code>") --> F["Output<br>rendering"]
    end
    
    %% style A fill:#e1f5fe
    style A fill:#ffebee,color:#000,font-size:18px
    style C fill:#e8f5e8,color:#000,font-size:18px
    style E fill:#fff3e0,color:#000,font-size:18px
```
...
graph TB
    ...
    style A color:#000,font-size:18px
...
graph TD
    ...
    subgraph Pred["<strong>Predict</strong>"]
        Req("<code>request.post()</code>") --"Sends HTTP request to server API, waits for response, returns"--> PredR("<code>pred()</code>")
    end
    ...
    I4 --"<code>@reactive.event()</code>"--> Req
    
    ValsReact --"as <code>data_to_send</code>"--> Req
    ValsReact --> ValsOut("<code>vals_out()</code>") -- "<code>data = vals()</code>" --> RenPrnt[\"<strong>@render.text()</strong><br><code>data</code>"\]
    PredOutReact --> RenTxt[\"<strong>@render.text()</strong><br><code>result</code>"\]

    %% Style for functions %%
    style Req fill:#FFF,color:#000,rx:20,ry:20,font-size:18px
    style PredR fill:#ffebee,color:#000,font-size:18px
    
    %% Style for reactives %%
    style ValsReact fill:#e1f5fe,stroke:#000,color:#000,stroke-width:1px,rx:10,ry:10,font-size:16px
    style ValsOut fill:#e1f5fe,stroke:#000,color:#000,stroke-width:1px,rx:10,ry:10,font-size:16px
    style PredOutReact fill:#fff3e0,stroke:#000,color:#000,stroke-width:1px,rx:10,ry:10,font-size:16px
