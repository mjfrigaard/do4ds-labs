...
Below are some pointers so you don't make the same mistakes I made:
...
I wrote `mod-api.py` to run the Vetiver/FastAPI in a dedicated Positron/Terminal session. My version deviates quite a bit from the original Quarto document, but it's essentially creating the same thing.
...
The bottom section of the UI shows the **Schema Information**:
...
Now that we've confirmed our API is running, we can launch our Shiny (for Python) app from a second Terminal (or RStudio/Positron session).
