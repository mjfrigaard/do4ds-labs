...
flowchart TD
 ...
 subgraph Pred["<strong>Predict</strong>"]
        PredR("<code>pred()</code>")
        Req("<code>request.post()</code>")
  end
 subgraph PredOutReact["<strong>pred_out()</strong>"]
    ...
    ValsReact --> ValsOut("<code>vals_out()</code>")
    ValsOut -- "<code>data = vals()</code>" --> RenPrnt[\"<strong>@render.text()</strong><br><code>data</code>"\]
