...
def server(input, output, session):
    ...
    def vals():
        ...
        for err in errors:
            logging.warning(f"Validation error - Session: {session_id} - {err}")
    ...
    def api_health_check():
        try:
            ...
            if r.status_code == 200:
                ...
                return f"API ping failed: {r.status_code}"
            ...
            return "API health check timeout"
    ...
    def pred():
        ...
        try:
            ...
            request_times.set(current_times[-10:])

            if r.status_code == 200:
                prediction = parse_prediction(r.json())
                ...
                logging.error(f"Prediction request failed - Session: {session_id} - status: {r.status_code}")
            ...
            logging.warning(f"API timeout - Session: {session_id}")
            error_count.set(error_count() + 1)
            return "Request timed out - API may be overloaded"
        except Exception as e:
    ...
    def log_file_content():
        try:
            with open(log_file_path, "r") as f:
                lines = f.readlines()
                return {"lines": lines, "last_modified": datetime.now(), "total_lines": len(lines)}
        except Exception as e:
            logging.error(f"Error reading log file - Session: {session_id} - error: {str(e)}")
            return {"lines": [], "last_modified": datetime.now(), "total_lines": 0}
    ...
    def pred_out():
        ...
        if isinstance(result, (int, float)):
            ...
            logging.info(f"Displaying prediction - Session: {session_id} - display_value: {display_value}")
    ...
    def recent_logs_display():
        ...
        if lines:
            ...
            return "\n".join(line.rstrip() for line in recent)
        return "No logs available yet..."

    @render.text
    def log_timestamp():
        return log_file_content()["last_modified"].strftime("%Y-%m-%d %H:%M:%S")
