...
The first time we connect from a new machine, SSH has never seen this server before, so it asks us to verify the host key. The output looks something like the block below (the real addresses and fingerprint are shown here as placeholders):
...
The `The authenticity of host ... can't be established.` is the **host key verification**. This means our client has no record of this server yet, so it can't confirm we're talking to the right machine. 

We answer `yes` only after checking the fingerprint against the value AWS reported for the instance. After confirming, we see the key has been added to the list of known hosts.

```{verbatim}
Warning: Permanently added 'YOUR_PUBLIC_IP_ADDRESS' (ED25519) to the list of known hosts.
```

Once we accept it, SSH stores it in `~/.ssh/known_hosts` and won't prompt again unless the key changes.
...
Newer OpenSSH versions try a post-quantum key exchange by default. When the server doesn't support it, SSH prints a warning, then falls back to a classical algorithm (this is informational, not an error).

#### Successful login

In the Terminal, we'll see `Last login: ...` and the `[ec2-user@ip-... ~]$` prompt. The shell prompt changing to the instance's user and hostname confirms we're now on the EC2 server.

```{verbatim}
Last login: <day> <month> <dd> <hh:mm:ss> <yyyy> from YOUR_PUBLIC_IP_ADDRESS
   ,     #_
   ~\_  ####_        Amazon Linux 2
  ~~  \_#####\
...
[ec2-user@ip-YOUR_INSTANCE_HOSTNAME ~]$
```

We can confirm the new key works because the login completes without asking for a password or the old key.

```{r}
#| label: co_box_placeholders
#| echo: false
#| results: asis
#| eval: true
co_box(
  ...
  hsize = "1.10",
  size = "1.00",
  header = "Keep these private!",
  fold = FALSE,
  contents = "

The public IP, internal hostname, host key fingerprint, and login timestamps above are placeholders. Never publish your instance's real address or host key fingerprint; the fingerprint is meant to be verified out of band, and a live IP invites scanning.")
```
