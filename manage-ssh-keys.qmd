...
This is the same process we followed in [Creating a regular SSH key for routine access](aws-ssh-ec2.qmd#creating-a-regular-ssh-key-for-routine-access), just run on the new machine. On the MacBook, create a key pair:
...
From a machine that can already reach the instance, connect and append the MacBook's public key to `authorized_keys`:
...
#### Logging in from another machine (notes)
