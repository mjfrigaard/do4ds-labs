...
First we should think about what logs (if any) we want displayed in the UI. I'm a big fan of having verbose logs and making them accessible in the UI. 

### Session Token

We'll start by putting a `log_status` output in the upper right-hand corner of the display.
...
This output is generated from the `session$token`:
...
![Penguin Parameters and Predicted Mass](img/lab4-predictions-wireframe.png)
...
In the **System Status** display, I want to 1) send a `ping` to the API and return the result as **API Status**, 2) print a subset of **Recent Logs** from the log file we specified above in the `logger::log_appender()` function, and 3) include a timestamp displaying when the log file was **Last updated**. 

![](img/lab4-system-status-wireframe.png){width='100%' fig-align='center'}

#### UI

The **System Status** section in the UI can be included as a `card()` in the `layout_columns()` (as long as we set the `col_widths` and a fixed `height`):
...
Recall that all the events we've captured with the  `logger::log_*()` functions are written to the log file (`shiny_log.log`). If we want to display some of the most recent logs in the UI, we can use Shiny's [`reactiveFileReader()`](https://shiny.posit.co/r/reference/shiny/latest/reactivefilereader.html) and configure it to poll the log file every 1 second (1000ms) for changes:
...
flowchart TD
 ...
 subgraph User["Log Generation"]
        ...
        R(["API<br>calls"])
        S(["System<br>events"])
        %% T@{ shape: rect, label: "<code>logger::log_</code>*<br>functions" }
        T["<code>logger::log_</code>*<br>functions"]
  end
 ...
 subgraph FileSys["File System"]
        %% A@{ shape: lin-doc, label: "<code>shiny_app.log</code>" }
        A[\"Log file: <code>shiny_app.log</code>"\]
 ...
 subgraph ReactFile["Reactive File"]
        %% B@{ shape: subproc, label: "<code>reactiveFileReader()</code>" }
        B{{"<code>reactiveFileReader() </code>"}}
        C{"Log file<br>exists?"}
        D("<strong>lines<br>last_mod<br>total_lines</strong>")
        E("<strong>lines</strong>:<code>character(0)</code><br><strong>last_mod</strong>:<code>Sys.time()</code><br><strong>total_lines</strong>:<code>0</code>")
  end
    ...
    style Q fill:#FFF,color:#000,rx:20,ry:20,font-size:18px
    style R fill:#FFF,color:#000,rx:20,ry:20,font-size:18px
    style S fill:#FFF,color:#000,rx:20,ry:20,font-size:18px
    
    style T fill:#4ac590,color:#000,rx:20,ry:20,font-size:18px
    
    style A fill:#FFF,color:#000,rx:20,ry:20,font-size:18px
    
    style A1 fill:#62d3fc,color:#000,rx:12,ry:12,font-size:18px
    style A2 fill:#62d3fc,color:#000,rx:12,ry:12,font-size:18px

    style B fill:#d08300,stroke:#d08300,color:#fff,rx:12,ry:12,font-size:18px
    
    style C fill:#f2b857,stroke:#000,color:#000,font-size:18px
    style D fill:#f2b857,stroke:#000,color:#000,rx:20,ry:20,font-size:18px
    style E fill:#f2b857,stroke:#000,color:#000,rx:20,ry:20,font-size:18px
...
To make this display more manageable, we'll limit the number of extracted logs to the five most recent line (unless there are less than five total lines in the log file). We'll format the logs to be displayed in the  `verbatimTextOutput()` and include a message if the log file is empty. 
...
flowchart TD
  ...
  A{{"<code>reactiveFileReader()</code> "}}

 subgraph LogProc["Log Processing"]
        H(["Reads <code>shiny_app.log</code>"])
        I{"More than 5 lines in <code>total_lines</code>?"}
        J("Print last 5 lines")
        K("Print all lines")
        L["Format: <code>paste(lines,</code><br><code>collapse = '\\n')</code>"]
        M["Return:<br><code>'No logs available'</code>"]
  end
 ...
 subgraph UI["UI Rendering"]
	 N[/"<code>log_file_content()</code>"/]
	 subgraph P["Display in <code>div</code>"]
		 O["<code>verbatimTextOutput(recent_logs)</code>"]
	 end
  end
  ...
  style A fill:#d08300,stroke:#d08300,color:#ffffff,rx:12,ry:12,text-align:center
  style H fill:#216654,stroke:#216654,color:#ffffff,rx:12,ry:12,text-align:center
  
  style L fill:#40915d,stroke:#216654,color:#ffffff,rx:12,ry:12
  style M fill:#40915d,stroke:#216654,color:#ffffff,rx:12,ry:12
  
  style N fill:#772f98,color:#fff,text-align:center
  style O fill:#cc66d3,color:000,text-align:center
...
If the API is running, we should see the results of the API health check and the app start-up/initialization:
...
When we click **Predict** and send the request to the API, the log file is updated with: 
