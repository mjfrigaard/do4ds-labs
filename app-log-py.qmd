...
#| code-fold: false 
    ...
    def api_health_check():
        ...
        try:
            logging.debug(f"Checking API health - Session: {session_id}") #<1>
            start_time = time.time() #<2>
            
            r = requests.get(ping_url, timeout=5) #<3>
            response_time = time.time() - start_time #<4>
            
            if r.status_code == 200: #<5>
                logging.info(f"API health check successful - Session: {session_id} - response_time: {response_time:.3f}s")
                return f"✅ API is running (ping: {r.json()}) - {response_time:.2f}s" #<5>
            else: #<6>
                logging.warning(f"API ping failed - Session: {session_id} - status: {r.status_code}")
                return f"⚠️ API ping failed: {r.status_code}" #<6>
