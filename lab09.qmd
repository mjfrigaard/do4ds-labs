...
- **Options/Flags:** Modify behavior, prefixed with `-` (single) or `--` (double)
...
- **Relative:** No leading `/`. Interpreted from your current directory. Use `..` to go up one level, `.` for current directory.
...
graph LR
    ...
    CD["<code>cd</code> command<br/>Change directory first"]
    ABS["Absolute path<br/>Specify full path"]
    REL["Relative path<br/>Specify from current location"]
    PWD["<code>pwd</code><br/>Check where you are"]
    Dot["<code>./file</code><br/>Run from current directory"]
...
- `cd /path/to/directory`: Change to that directory, then run commands
- `/absolute/path/to/file`: Run the file with absolute path directly
- `relative/path/file`: Run file relative to current location
- `./script.sh`: Run script in current directory (explicit)
- `pwd`: Always check your current location first
