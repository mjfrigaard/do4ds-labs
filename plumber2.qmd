...
Since the original writing of [do4ds](https://do4ds.com/), the [`plumber`](https://www.rplumber.io/) package underwent an upgrade to [`plumber2`](https://plumber2.posit.co/). I've summarized the changes to our original API (along with the inclusion of the [`logger` package](https://daroczig.github.io/logger/)) below. 
...
1. `model.R` builds the model used for the predictions.  
2. `plumber.R` runs the API.    
3. `app.R` launches the Shiny app. 
