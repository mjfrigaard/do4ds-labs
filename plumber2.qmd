...
Test with `curl` in the Terminal:[^port]

[^port]: We're using port `8081` for our `plumber2` API (`8080` is being used by the `plumber` API).
...
![`plumber2` UI](img/lab3-plumber2-router-positron.png){width='100%' fig-align='center'}

If we click on the **API Servers**, we will see the url our `plumber2` api is running on: 

![`plumber2` API server](img/lab3-plumber2-api-server.png){width='100%' fig-align='center'}
...
![Shiny app predictions](img/lab3-plumber2-shiny-prediction.png){width='100%' fig-align='center'}
...
We'll cover how to keep detailed logs on APIs and Apps in [R App Logging](app-log-r.qmd).
