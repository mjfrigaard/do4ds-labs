...
The R equivalent lives in [`_labs/lab05/cicd/R/penguinpredictor/`](https://github.com/mjfrigaard/do4ds-labs/tree/main/_labs/lab05/cicd/R/penguinpredictor).[^r-pkg] It follows the same pattern as the Python package: pure functions are extracted from the Shiny app into a proper R package so they can be tested with `testthat` independently of the UI.

[^r-pkg]: The package name `penguinpredictor` follows R conventions (no underscores in package names).
...
└── app.R
```
1. `validate_inputs()`, `encode_inputs()`, `parse_prediction()` — the same three functions extracted from the app
2. 21 `testthat` tests covering valid inputs, boundary conditions, encoding, and response parsing
...
The workflow files in [`.github/workflows/python-tests.yml`](https://github.com/mjfrigaard/do4ds-labs/tree/main/_labs/lab05/cicd/Python/penguin_predictor) and [`.github/workflows/r-tests.yml`](https://github.com/mjfrigaard/do4ds-labs/tree/main/_labs/lab05/cicd/R/penguinpredictor) are included as references and won't execute when this book is pushed to GitHub. 
